...
Computer simulations of single-cell technologies—such as single-cell RNA sequencing (scRNA-seq), single-cell ATAC sequencing (scATAC-seq), CITE-seq, and single-cell methylation—are widely used in biological research. They help scientists test new computational tools, optimize experimental protocols, and study statistical models. However, an important question is always: *how realistic is the simulated data compared to genuine biological experiments?*
...
1. **Real experimental reference data** (`ref_rna`, `ref_atac`, CITE-seq protein, or methylation $\beta$-values)
2. **Simulated synthetic data** (`sim_rna`, `sim_atac`, CITE-seq protein, or methylation $\beta$-values)
...
4. **Outputs:** A clean summary table, normalized scores, ranking leaderboards, and publication-ready plots.
...
### 3.2 Visual Mapping to Publication Bubble Matrices
...
`scSimEval` provides built-in plotting functions designed to produce publication-grade figures for high-impact scientific journals.
...
```r
# Generate publication-grade benchmark bubble matrix
...
For researchers who prefer an interactive graphical interface rather than writing R code, `scSimEval` includes an embedded Shiny web application. You can explore benchmark results, filter metrics, view bubble plots, and export publication-ready figures directly in your web browser:
...
## 15. Extension to Other Single-Cell Technologies

Because all 62 evaluation measures operate on standard numeric count or continuous matrices, `scSimEval` can be applied directly to other single-cell multiomics technologies:
* **CITE-seq / REAP-seq:** Pass RNA counts as Modality 1 and antibody-derived tag (ADT) protein counts as Modality 2.
* **Single-Cell DNA Methylation:** Pass single-cell methylation percentages ($\beta$-values between 0 and 1) as numeric matrices.
* **Spatial Transcriptomics:** Pass gene expression counts as Modality 1 and spatial or morphological features as Modality 2.

---

## 16. Summary

`scSimEval` provides a unified, objective, and ground-truth-free evaluation toolkit for benchmarking single-cell and multiomics simulation methods. With 62 metrics organized into 8 clear categories, automated score normalization, and publication-ready visualizations, it enables researchers to select and validate simulation methods with confidence.

---

## 17. Authors and Citation
...
To cite `scSimEval` in publications:
...
For questions, feature requests, or bug reports, please contact the maintainer or visit the package repository.
